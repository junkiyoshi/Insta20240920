...
void ofApp::draw() {
	...
	for (int k = 0; k < 12; k++) {
		...
		for (int i = 0; i < 40; i++) {
			...
			auto radius = ofMap(sin((ofGetFrameNum() + i) * 0.2 + PI * (k / 6)), 0, 1, 280, 320);
			auto next_radius = ofMap(sin((ofGetFrameNum() + i + 1) * 0.2 + PI * (k / 6)), 0, 1, 280, 320);
			...
			right.push_back(location + glm::vec3(ofMap(i, 0, 40, 0, 6) * cos(theta + PI * 0.5), ofMap(i, 0, 40, 0, 6) * sin(theta + PI * 0.5), 0));
			left.push_back(location + glm::vec3(ofMap(i, 0, 40, 0, 6) * cos(theta - PI * 0.5), ofMap(i, 0, 40, 0, 6) * sin(theta - PI * 0.5), 0));
		...
		int color_index = k % color_list.size();
		...
		ofSetColor(ofColor(color_list[color_index], 64));
		mesh.draw();
	}
